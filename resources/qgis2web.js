...
map.getView().fit([12529153.299003, -825146.485813, 12533759.660003, -821999.732782], map.getSize());
...
//layer search - cari bangunan desa (16 titik penting)
(function () {
    var buildingsLayer = lyr_TitikPentingDesa_15;
    var buildingsSource = buildingsLayer.getSource();
    var buildingFeatures = buildingsSource.getFeatures();

    function showFeaturePopup(feature, layer) {
        var keys = feature.getKeys();
        var popupText = '<ul><li><table>';
        popupText += '<a><b>' + layer.get('popuplayertitle') + '</b></a>';
        popupText += createPopupField(feature, keys, layer);
    ...
    searchElement.className = 'search-layer ol-unselectable ol-control';

    var searchButton = document.createElement('button');
    searchButton.type = 'button';
    searchButton.className = 'fa fa-search';
    searchButton.title = 'Cari bangunan desa';
    searchElement.appendChild(searchButton);

    var searchForm = document.createElement('form');
    searchForm.onsubmit = function (e) { e.preventDefault(); return false; };
    var searchInput = document.createElement('input');
    searchInput.className = 'search-layer-input-search';
    searchInput.type = 'text';
    searchInput.placeholder = 'Cari bangunan...';
    ...
    searchElement.addEventListener('touchstart', stopMapPropagation);
    searchElement.addEventListener('mousedown', stopMapPropagation);

    var items = buildingFeatures.map(function (feature, index) {
        return { text: feature.get('Nama Bangunan/Fasilitas'), value: index };
    });

    if (typeof horsey === 'function') {
        horsey(searchInput, {
            source: [{ list: items }],
            getText: 'text',
            getValue: 'value',
            ...
            predictNextSearch: function (info) {
                var feature = buildingFeatures[info.selection.value];
                if (!feature) { return; }
                var geometry = feature.getGeometry();
                ...
                showFeaturePopup(feature, buildingsLayer);
            }
        });
    }
})();
